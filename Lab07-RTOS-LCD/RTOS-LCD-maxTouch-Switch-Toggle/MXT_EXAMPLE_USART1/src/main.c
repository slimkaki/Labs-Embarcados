...
typedef struct {
	...
	uint8_t status;      // Status do botao
} t_but;

void draw_button_new(t_but but); // Botao LCD

int process_touch(t_but botoes[], touchData touch); // identifica o botao apertado
...
int process_touch(t_but botoes[], touchData touch){
	printf("identificando botao: ");
	if ((touch.x > botoes[0].x-botoes[0].width/2) && (touch.x < botoes[0].x+botoes[0].width/2) && (touch.y > botoes[0].y-botoes[0].height/2) && (touch.y < botoes[0].y+botoes[0].height/2)) {
		// tocou no but0
		printf("0\n");
		return 0;
	}
	else if ((touch.x > botoes[1].x-botoes[1].width/2) && (touch.x < botoes[1].x+botoes[1].width/2) && (touch.y > botoes[1].y-botoes[1].height/2) && (touch.y < botoes[1].y+botoes[1].height/2)) {
		// tocou no but1
		printf("1\n");
		return 1;
	}
	else if ((touch.x > botoes[2].x-botoes[2].width/2) && (touch.x < botoes[2].x+botoes[2].width/2) && (touch.y > botoes[2].y-botoes[2].height/2) && (touch.y < botoes[2].y+botoes[2].height/2)) {
		// tocou no but2
		printf("2\n");
		return 2; 
	} else if ((touch.x > botoes[3].x-botoes[3].width/2) && (touch.x < botoes[3].x+botoes[3].width/2) && (touch.y > botoes[3].y-botoes[3].height/2) && (touch.y < botoes[3].y+botoes[3].height/2)) {
		// tocou no but3
		printf("3\n");
		return 3;
	} else if ((touch.x > botoes[4].x-botoes[4].width/2) && (touch.x < botoes[4].x+botoes[4].width/2) && (touch.y > botoes[4].y-botoes[4].height/2) && (touch.y < botoes[4].y+botoes[4].height/2)) {
		// tocou no but4
		printf("4\n");
		return 4;
	}
	else {
		// tocou em nenhum botao
		return -1;
	}
}
...
void task_lcd(void){
	...
	// Vetor de Botoes
	t_but botoes[] = {but0, but1, but2, but3, but4}; // Vetor de structs
